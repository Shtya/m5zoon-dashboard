...
export default function page({params}) {
	...
	const { register, errors, trigger, clearErrors, setError, getValues, setValue, submit, watch, reset } = hookSignUp();
		...
		const breadcrumbItems = [
			...
			{ label: t('add_new_product'), href: null },
		];
    ...
    return (
        <div className='pb-[40px] pt-[20px]'  >
            ...
            <div className=' cShadow min-h-[100px] @max-xl:justify-center  flex-wrap bg-white p-[20px] rounded-[10px]  flex items-center justify-between gap-[10px]  '>
				<div>
                    <h1 className=' @max-lg:text-center text-[#0f1728] dark:text-[#fff] duration-300  text-2xl font-normal'> {t('add_new_product')} </h1>
			...
			<div className=' grid grid-cols-3 @max-xl:grid-cols-2 @max-lg:!grid-cols-1 mt-[20px] gap-[20px] '>
				<div className=' h-fit p20 @xl:col-span-2 grid grid-cols-2  gap-[30px] cShadow  bg-white rounded-[10px]' >
					<Input cn={" @max-xl:col-span-2 "} cnInput={'!h-[50px] !bg-[#f7f7f7] !border-[#ebecee] dark:!border-[#263358] '} KEY={'item_name'}             error={errors?.item_name}              type={'text'}       register={register('item_name')}           label={t('item_name')}   place={t("item_name")} />
					<Input cn={" @max-xl:col-span-2 "} cnInput={'!h-[50px] !bg-[#f7f7f7] !border-[#ebecee] dark:!border-[#263358] '} KEY={'wholesale_price'}       error={errors?.wholesale_price}        type={'text'}       register={register('wholesale_price')}     label={t('wholesale_price')}   place={t("wholesale_price")} />
					...
					<Input cn={" @max-xl:col-span-2 "} cnInput={'!h-[50px] !bg-[#f7f7f7] !border-[#ebecee] dark:!border-[#263358] '} KEY={'piece_count'}           error={errors?.piece_count}            type={'text'}       register={register('piece_count')}         label={t('piece_count')} place={t('piece_count')} />
					<Input cn={" col-span-2 "} cnInput={'!h-[50px] !bg-[#f7f7f7] !border-[#ebecee] dark:!border-[#263358] '} KEY={'image_files_link'}      error={errors?.image_files_link}       type={'text'}       register={register('image_files_link')}    label={t('image_files_link')} />
					{/* <Input  cnInput={'!h-[50px] !bg-[#f7f7f7] !border-[#ebecee] dark:!border-[#263358] '} KEY={'product_description'}   error={errors?.product_description}    type={'text'}       register={register('product_description')}  /> */}
