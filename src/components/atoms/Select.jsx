...
const Select = ({cnSelect ,label , cnInput , dataAos , required , error , trigger , watch , KEY, setValue, icon, data, cn, place }) => {
  ...
  return (
    <div ref={selectRef} data-aos={dataAos}  className={`${cn} select z-[100] relative flex  flex-col gap-[15px]  w-full `}>
      ...
      <div onClick={() => setShow(!show)} className={` ${cnSelect} px-[10px] rounded-lg border border-[#ececeb] duration-300 cursor-pointer w-full h-[55px] flex justify-between items-center`}>
        ...
        <Image className={`${show ? 'rotate-[180deg]' : ''} duration-300 `} src={"/icons/arrow-top-black.png"} alt='' width={12} height={12} />
      </div>

        <div style={{direction:"ltr"}} className={`almarai ${show ? 'max-h-[200px] opacity-100' : 'max-h-0 opacity-50 '} z-[10000]   overflow-auto  duration-300 shadow-md  ease-in-out absolute top-[110%] w-full bg-white `}>
          {data.map((e, i) => (
