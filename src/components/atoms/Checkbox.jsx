...
const Checkbox = ({ watch, dataAos, cn, KEY, label, setValue, err, cnLabel }) => {
    ...
    return (
        <div data-aos={dataAos} className={`form-group  mt-5 flex items-center  gap-3 ${cn}`}>
            ...
            <div onClick={handleCheckboxChange} className={`w-[20px] h-[20px] flex-none border-[1px] ${isChecked ? 'border-white bg-trnasparent' : 'border-black bg-white'} rounded-[4px] cursor-pointer flex items-center justify-center`}>
...
export const CheckBoxTable = ({ KEY, cn, onclick }) => {
    return (
        <div className={`flex items-center justify-center relative`}>
            <input id={KEY} onClick={() => onclick(KEY)} className={`${cn} peer checked:bg-primary duration-300 checked:!border-primary checked:shadow-md appearance-none  w-[20px] h-[20px] border-black border-[1px] rounded-[4px] cursor-pointer  `} type='checkbox' />
