...
export default function Input({ required , delay , place , dataAos , error , cnInput , cn , label , type , icon , KEY , register , cnLabel}) {
  ...
  const toggleInputType = () => {
	...
	<div data-aos={dataAos} data-aos-delay={delay} className={`${cn} flex flex-col  gap-[15px] max-md:gap-[10px]  relative `} >
		...
		<div  className={`h-[45px]   ttttt   relative overflow-hidden rounded-[6px] !border-[1px] border-[#ececeb] dark:border-border1 w-full ${cnInput} `} >
			<input className={` text-sm font-normal placeholder:text-[#6b6b6b] placeholder:dark:text-white text-[#6b6b6b] dark:text-[#fff] bg-transparent dark:bg-[#1F2A4A]  w-full ${icon ? "ltr:pl-[40px] rtl:pr-[40px]" : "" } px-[10px] h-[45px] outline-none `} {...register} id={KEY} placeholder={place} type={inputType} />
