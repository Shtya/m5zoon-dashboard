...
export default function Input({ required , delay , place , dataAos , error , cnInput , cn , label , type , icon , KEY , register , cnLabel}) {
  ...
  const toggleInputType = () => {
	...
	<div data-aos={dataAos} data-aos-delay={delay} className={`${cn} flex flex-col  gap-[15px] max-md:gap-[10px]  relative `} >
		...
		<div  className={`h-[55px]  relative overflow-hidden rounded-lg border-[2px] border-[#ececeb]  w-full ${cnInput} `} >
			<input className={`text-[#637381] text-base font-normal bg-transparent  w-full ${icon ? "ltr:pl-[40px] rtl:pr-[40px]" : "" } px-[10px] h-full outline-none `} {...register} id={KEY} placeholder={place} type={inputType} />
