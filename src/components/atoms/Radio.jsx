...
const Radio = ({ setError , watch  , setValue, KEY, cn ,  data }) => {
  ...
  return (
    <div  className={`${cn} mt-[20px] flex-wrap flex items-center gap-[10px] `}>
      {
        data?.map((e,i)=>(
          <div key={i} onClick={()=> handleChange(e.value) } className={` ${checked == e.value ? "!bg-secondery text-white scale-y-[1.05] " : ""} text-secondery hover:text-white cursor-pointer hover:bg-secondery duration-300 relative max-w-full  w-full h-[60px] px-2 py-4 bg-[#f9fafb] rounded-lg border border-[#eeeeee] flex gap-[5px] max-sm:flex-col max-sm:h-fit justify-start items-center `} >
